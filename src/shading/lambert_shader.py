...
class LambertShader(Shader):
    def __init__(self, lights, ambient_ray_count, ambient_coefficient, ambient_occlusion) -> None:
        super().__init__(lights, ambient_ray_count, ambient_coefficient, ambient_occlusion)

    def calculate_light(self, objects, hitPoint: "Point3f", normal: "Vector3f"):
        rayCount = 0
        ambient_rate, sentRay = self.calculate_ambient_occlusion(objects, hitPoint) # Calculate ambient occlusion as first step.
        rayCount += sentRay
        ...
        for l in self.lights:
            ...
            for obj in objects:
                ...
                temp_dist, _, _, _, sentRay = obj.intersect(ray)
                rayCount += sentRay
